...
import { Wallet, Vote, ShoppingCart, Layers, Home, Settings } from 'lucide-react';
...
type TemplateType = 'token-receive' | 'voting' | 'nft-marketplace' | 'sell-tokens' | 'gambling' | 'gaming' | null;
      ...
      default:
        return (
          <div className="flex flex-col items-center justify-center text-center p-6 h-full">
            <p className="text-lg font-medium mb-4">Select a template to get started</p>
            <p className="text-muted-foreground max-w-md">
              Choose a template from the sidebar to begin building your Web3 application.
            </p>
          </div>
        );
    }
      ...
      <header className="bg-background/30 backdrop-blur-md border-b border-white/10 py-4 px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Button 
            ...
            className={`gradient-border ${selectedTemplate ? 'animate-glow' : ''} bg-builder-accent hover:bg-builder-accent/80`}
          >
            Generate Link
          </Button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 border-r border-white/10 bg-background/30 backdrop-blur-md p-4 overflow-y-auto hidden md:block">
          ...
          <div className="space-y-3">
            <Card 
              className={`p-3 cursor-pointer hover:shadow-md transition-shadow ${selectedTemplate === 'token-receive' ? 'gradient-border' : ''}`}
            ...
            <Card 
              className={`p-3 cursor-pointer hover:shadow-md transition-shadow ${selectedTemplate === 'voting' ? 'gradient-border' : ''}`}
            ...
            <Card 
              className={`p-3 cursor-pointer hover:shadow-md transition-shadow ${selectedTemplate === 'nft-marketplace' ? 'gradient-border' : ''}`}
            ...
            <Card 
              className={`p-3 cursor-pointer hover:shadow-md transition-shadow ${selectedTemplate === 'sell-tokens' ? 'gradient-border' : ''}`}
            ...
            <Card 
              className={`p-3 cursor-pointer hover:shadow-md transition-shadow ${selectedTemplate === 'gambling' ? 'gradient-border' : ''}`}
            ...
            <Card 
              className={`p-3 cursor-pointer hover:shadow-md transition-shadow ${selectedTemplate === 'gaming' ? 'gradient-border' : ''}`}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto builder-grid">
            <div className="container mx-auto">
              <div className="mb-6">
                <h2 className="text-xl font-medium mb-3 text-gradient">Template Preview</h2>
                <p className="text-sm text-muted-foreground">Select a template from the sidebar to preview.</p>
              </div>
              ...
              {link && (
                <div className="mt-6 p-4 neo-blur rounded-lg animate-slide-up">
                  ...
                  <div className="flex mt-2">
                    <Input id="generated-link" value={link} readOnly className="flex-1 bg-background/50" />
                    <Button 
                      variant="secondary" 
                      className="ml-2"
                      onClick={() => {
          ...
          <div className="md:hidden border-t border-white/10 bg-background/30 backdrop-blur-md p-2">
            <div className="flex justify-around">
              ...
              <Button variant={activeTab === 'more' ? 'default' : 'ghost'} size="sm" className="flex-1" onClick={() => setActiveTab('more')}>
                <Layers size={20} />
              </Button>
